...
const StudentCard = ({ student, index, onActivityClick }: StudentCardProps) => {
  ...
  return (
    ...
    >
      <Card
        id={student.name}
        className={`hover:shadow-lg transition-shadow duration-300 ${student.name === login ? "border-2 border-purple-600" : ""}`}
      >
        <CardContent className="flex items-center p-4">
            ...
            {student.correctionPercentage !== 420 && student.correctionTotal >= 10 && (
              <>
                <p className="text-sm text-gray-500 mt-1">
                  Correction: {student.correctionPercentage}% ({student.correctionTotal - student.correctionPositive}/
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="flex gap-2 mt-2">
              <Button variant="outline" size="sm" onClick={() => onActivityClick(student)}>
...
export default function RankingList() {
  ...
  const [updatedAt, setUpdatedAt] = useState(
    user.time ? format(new Date(user.time[0].updatedAt), "dd-MM-yyyy HH:mm:ss") : "N/A",
  )
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(async () => {
      ...
      const user = JSON.parse(localStorage.getItem("user") || "{}")
      setUpdatedAt(user.time ? format(new Date(user.time[0].updatedAt), "dd-MM-yyyy HH:mm:ss") : "N/A")
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (filter !== "all") {
      result = result.filter((student) => student.level === Number.parseInt(filter))
    }

    if (year !== "all") {
      result = result.filter((student) => student.year === Number.parseInt(year))
    }

    if (campus !== "all") {
      result = result.filter((student) => student.campus === campus)
    }
    ...
    switch (sort) {
      ...
      case "correction":
        result.sort((a, b) => {
          if (
            (a.correctionPercentage === 420 && b.correctionPercentage === 420) ||
            (a.correctionTotal < 10 && b.correctionTotal < 10)
          ) {
            return 0
          }
      ...
      case "activity":
        result.sort((a, b) => {
          const getActivityTime = (student: Student) => {
            ...
            if (
              typeof student.activityData === "object" &&
              !Array.isArray(student.activityData) &&
    ...
    return result
  }, [students, searchQuery, filter, sort, year, sortDirection, previewLevel, campus])
    ...
    <div className="max-w-7xl mx-auto px-4">
      ...
      >
        ...
        <div className="flex items-center space-x-4">
          <Button variant="outline" size="sm" onClick={toggleSortDirection} className="flex items-center space-x-2">
          ...
          <div className="space-y-4">
            {user.login && filteredStudents.findIndex((student) => student.name === user.login) !== -1 && (
              <p className="text-sm text-gray-500">
                Your position :{" "}
                <strong>{filteredStudents.findIndex((student) => student.name === user.login) + 1 || "N/A"}</strong> /{" "}
                ...
                >
                  (page{" "}
                  {Math.ceil(
                    (filteredStudents.findIndex((student) => student.name === user.login) + 1) / ITEMS_PER_PAGE,
                  ) || "N/A"}
                  )
                </a>
              </p>
