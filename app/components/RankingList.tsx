...
export default function RankingList() {
  ...
  const filteredStudents = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "time":
        result.sort((a, b) => {
          const getLatestTime = (student: Student) => {
            if (!student.activityData) return 0
            if (Array.isArray(student.activityData)) {
              return student.activityData[student.activityData.length - 1]?.value || 0
            }
            if ("dailyHours" in student.activityData) {
              const dailyHours = student.activityData.dailyHours
              return dailyHours[dailyHours.length - 1]?.value || 0
            }
            return 0
          }
          ...
          return sortMultiplier * (timeB - timeA)
        })
        break
      default:
        result.sort((a, b) => sortMultiplier * (b.level - a.level))
        ...
        <div className="flex items-center space-x-4">
          ...
          <FilterSort
            ...
            sortOptions={[
              ...
              { value: "correctionPoints", label: "Correction Points" },
              { value: "wallet", label: "Wallet" },
              { value: "blackhole", label: "Blackhole" },
              { value: "time", label: "Activity Time" },
            ]}
          />
        </div>
      </div>
